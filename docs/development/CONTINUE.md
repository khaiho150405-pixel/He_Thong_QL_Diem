...
- Yêu cầu mới nhất: Phase 5 đã merge qua PR #7 tại `7cebf3c`; tiếp tục Phase 6 theo AGENTS.md. Phần 1 hardening đã xanh ở commit `bc73f6c`; phần 2 đang thêm backup/verify/restore rehearsal và CI. Chưa deploy production hoặc phát hành store.
...
- Phase 3 phần 4 đã có API list/detail theo phạm vi phân công, signed URL ảnh private 300 giây và Flutter upload/polling/xem bằng chứng hai kênh. UI chưa có nút duyệt; kết quả máy vẫn không ghi điểm chính thức.
...
- Kiểm Phase 3 phần 4 dùng database mới `qld_phase3_part4_test` tại `127.0.0.1:55434`: đủ bảy migration từ rỗng, seed giả và bốn integration suite tuần tự đều đạt. Cụm ở `.local/demo-pg`; kiểm tiến trình/cổng trước khi khởi động lại.
...
Phase 3 phần 3 thêm migration `202609120001_recognition_results`; đã nâng cấp DB Phase 3 và cài đủ bảy migration từ DB rỗng. `pnpm check` đạt 12/12 unit/contract + build, Python đạt 9/9, `pnpm test:db` đạt và bốn integration suite PostgreSQL tuần tự đạt; luồng upload → worker → `CHO_DOI_CHIEU` giữ replay không trùng và `diem_thanh_phan` vẫn không có giá trị. `pnpm contracts:check` đạt sau khi ổn định thứ tự snapshot. CI `82b830e` và `b3454d4` đỏ tại Docker vì tag MinIO đã bị gỡ khỏi Docker Hub; compose chuyển cùng tag bất biến sang registry chính thức `quay.io` và giữ retry/log chẩn đoán. Commit `38a82de` đã xanh `foundation`, `ios`, `required-checks` tại [GitHub Actions run 34669443801](https://github.com/khaiho150405-pixel/He_Thong_QL_Diem/actions/runs/34669443801).
