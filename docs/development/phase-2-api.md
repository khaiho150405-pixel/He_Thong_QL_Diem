...
Review bắt buộc: nhập NULL/0.0; sửa nhiều ô; gửi lại cùng key; dùng key người khác; trộn cellId của bảng khác; phiên bị thu hồi; hai lần sửa/chốt cùng version; cố ý gây lỗi audit; chốt khi thiếu điểm; đồng bộ học sinh mới. Test backend nằm ở `apps/api/test/integration/gradebook-write.test.ts` và `gradebooks.test.ts`; test UI ở `apps/client_flutter/test/phase2_gradebooks_test.dart`. Giao diện dùng generated `GradebooksApi`, giữ version người dùng đã đọc và yêu cầu tải lại/đối chiếu khi API trả 409.
