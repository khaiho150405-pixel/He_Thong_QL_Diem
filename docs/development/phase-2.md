...
Phần 1–2 backend và phần 3 Flutter đã hoàn tất và được merge vào `main` qua PR #3, merge commit `24008b5`. Sau merge, integration suite danh mục đôi lúc xung đột với hai suite bảng điểm vì Node chạy ba tệp song song trên cùng PostgreSQL ở isolation `SERIALIZABLE`. Bản sửa sau merge đặt `--test-concurrency=1` cho cấp tệp; các ca cạnh tranh có chủ đích bên trong suite bảng điểm vẫn chạy song song.
...
Phần 3 kiểm local ngày 2026-09-11: `dart run melos run check` đạt analyzer và 13 tests; bốn widget test mới đạt ở 390px/1280px cho NULL/0.0, batch/version/idempotency, retry giữ nguyên key, conflict, lịch sử và chốt. `flutter build web --dart-define-from-file=config/development.json` đạt. Android local dừng trước biên dịch do Gradle/JVM trên máy không tạo được kết nối loopback; lỗi toolchain này cũng từng được ghi ở trạng thái Phase 0. CI commit `23df8fe` có foundation xanh nhưng smoke iOS chạm timeout 10 phút sau khi build simulator đạt; giới hạn riêng được tăng lên 15 phút. [Lượt follow-up commit `887155c`](https://github.com/khaiho150405-pixel/He_Thong_QL_Diem/actions/runs/34565168338) xác nhận foundation, Web, APK, Android emulator, iOS build/simulator và required-checks đều xanh.

Kiểm chứng hotfix sau merge: chạy song song ba integration suite nghiệp vụ trên PostgreSQL test tái hiện đúng 2 đạt/1 lỗi với API trả 500 khi tạo học sinh; chạy tuần tự cùng ba tệp đạt 3/3 và `pnpm test:db` đạt. Probe Redis/MinIO không thể chạy local do máy chưa có Docker và phải được GitHub Actions xác nhận.
