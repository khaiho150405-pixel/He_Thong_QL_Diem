...
psql --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" --set=ON_ERROR_STOP=1 \
  --set=migration_password="$MIGRATION_PASSWORD" --set=runtime_password="$RUNTIME_PASSWORD" --set=db_name="$POSTGRES_DB" <<'SQL'
...
GRANT CONNECT, CREATE ON DATABASE :"db_name" TO app_migration;
